...
CREATE OR REPLACE TABLE
 {{ params.ds_stg }}.sps_chnnl_onorder AS
SELECT
  ...
  COALESCE(SUM(CASE
        WHEN (DATE(base.sb_event_date_date, 'America/Denver')) = DATE_SUB(CURRENT_DATE(),INTERVAL 1 day) OR (DATE(base.sb_event_date_date, 'America/Denver')) = (DATE(dates.FISCAL_END_OF_WK, 'America/Denver')) THEN base.on_orders_cost
      ELSE
      NULL
    END
      ), 0) AS on_order_cost,
  COALESCE(SUM(base.on_orders_quantity ), 0) AS on_order_quantity,
  COALESCE(SUM(base.on_order_retail_value), 0) AS on_order_retail,
FROM (
  SELECT
...
WHERE
  DATE(base.sb_event_date_date) IN ( SELECT
